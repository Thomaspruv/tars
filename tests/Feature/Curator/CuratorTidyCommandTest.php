...
test('warns and fails when the curator is not configured and not scheduled', function () {
    $this->artisan('curator:tidy')->assertExitCode(1);
});

test('skips silently when the curator is not configured and scheduled', function () {
    $this->artisan('curator:tidy --scheduled')->assertExitCode(0);
});

test('processes the tidy mission manually', function () {
    ...
    $this->mock(CuratorAgent::class, function ($mock): void {
        ...
        $mock->shouldReceive('process')
            ->once()
            ->with(AgentRunTrigger::Manual, 'tidy')
            ->andReturn(AgentRun::factory()->create(['status' => AgentRunStatus::Success]));
    });

    $this->artisan('curator:tidy')->assertExitCode(0);
});

test('processes the tidy mission on schedule with the scheduled trigger', function () {
    AgentConfig::factory()->create(['agent_name' => 'curateur', 'enabled' => true]);
    ...
    $this->mock(CuratorAgent::class, function ($mock): void {
        ...
        $mock->shouldReceive('process')
            ->once()
            ->with(AgentRunTrigger::Scheduled, 'tidy')
            ->andReturn(AgentRun::factory()->create(['status' => AgentRunStatus::Success]));
    });
