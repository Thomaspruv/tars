...
#[Signature('curator:tidy {--scheduled : Skip silently instead of failing when the curator is not configured}')]
#[Description('Range le cerveau — mission B, ancrage/classement/fusion/archivage sur tout le vault')]
class CuratorTidyCommand extends Command
{
    public function handle(CuratorAgent $curatorAgent): int
    {
        ...
        $run = $curatorAgent->process($trigger, 'tidy');

        if (! $run || $run->status !== AgentRunStatus::Success) {
            $this->error('Échec du rangement.');

            return self::FAILURE;
        }

        $this->info('Rangement effectué.');
