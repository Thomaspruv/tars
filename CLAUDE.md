...
**Domain model**: `Goal` → `Milestone`/`Task`/`Event`/`Note`/`Decision`, with `Entity` (people/orgs/projects) cross-cutting most of those. `InboxItem` is the capture-first, triage-later staging table (see `App\Support\QuickAdd`). `Checklist`/`ChecklistItem` are freeform lists, distinct from `Task`. `Review` drives the periodic review flow. `AiProvider`/`AgentConfig`/`AgentRun` back the `reviewer`/`curateur`/`triage`/`planner` agents on the `/agents` screen (provider/model config, run history, manual + scheduled triggers). `App\Support\Triage\TriageAgent` turns `InboxItem`s into `InboxSuggestion`s (convert to task/event/goal, etc.); `App\Support\Planner\PlannerAgent` generates `PlannerProposal`/`PlannerProposalItem` scheduling proposals for a given period. There used to be a `LifeArea` model above `Goal`/`Entity` in this hierarchy; it was removed (2026-07-29) as unnecessary overhead — don't reintroduce a `life_area_id` column or a domain-of-life grouping concept without a fresh decision to do so.
...
**Google Calendar sync** (`/calendrier` page, "Connexions" section on `/reglages`): a single global `GoogleCalendarConnection` row (no per-user scoping, matching `AiProvider`/`Setting`) holds the OAuth tokens. `App\Support\GoogleCalendar\GoogleCalendarClient` talks to Google's OAuth + Calendar v3 REST endpoints directly via `Http` (no `google/apiclient` dependency); `GoogleCalendarSyncService` maps `Event` ↔ Google's event JSON and drives both directions. Push is automatic: `#[ObservedBy(GoogleCalendarSyncObserver::class)]` on `Event` dispatches `PushEventToGoogleCalendarJob`/`DeleteGoogleCalendarEventJob` on create/update/delete. Pull is polled by `google-calendar:sync` (`SyncGoogleCalendarCommand`, every 5 minutes via `routes/console.php`), using Calendar API `syncToken` incremental sync with a bounded full-resync fallback on `410 Gone`. Loop prevention: the pull path upserts `Event`s inside `Event::withoutEvents(...)`, and the push path persists `google_event_id`/`google_synced_at` via `forceFill()->saveQuietly()` — both bypass the observer so pulled/pushed writes don't re-trigger a sync in the other direction. `google_event_id`/`google_synced_at` are deliberately **not** in `Event`'s `#[Fillable]` — they're system-managed only.
